...
void update(int v, int tl, int tr, int l, int r, int x, int c)
{
	if (tr < l or r < tl)
		return;

	if (l <= tl and tr <= r and c == 1)
	{
		lazy[v] += x;
		ans[v] += (tr - tl + 1) * x;
		return;
	}
	if (l <= tl and tr <= r and c == 2)
	{
		lazy[v] = x;
		ans[v] = (tr - tl + 1) * x;
		return;
	...
	int tm = (tl + tr) / 2;
	update(v << 1, tl, tm, l, r, x, c);
	update(v << 1 | 1, tm + 1, tr, l, r, x, c);
